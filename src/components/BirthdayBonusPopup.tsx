...
import { Cake, CakeSlice, Star } from "lucide-react";

interface BirthdayBonusPopupProps {
  open: boolean;
  onClose: () => void;
  birthYear: number;
  points: number;
...
const BirthdayBonusPopup: React.FC<BirthdayBonusPopupProps> = ({ open, onClose, birthYear, points }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      try {
        ...
        console.error("Could not play birthday bonus sound", e);
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-gradient-to-b from-amber-100 to-yellow-200 p-6 rounded-lg border-4 border-yellow-400 shadow-xl max-w-sm">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          >
            ...
            >
              <CakeSlice className="h-10 w-10 text-purple-700" />
            </motion.div>
          </motion.div>
          ...
          <div>
            ...
            <p className="mt-2 text-amber-700 kids-text text-lg">
              ¡La matrícula tiene tu año de nacimiento ({birthYear})!
            </p>
...
export default BirthdayBonusPopup;
