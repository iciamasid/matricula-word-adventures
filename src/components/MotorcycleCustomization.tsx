...
const MotorcycleCustomization: React.FC = () => {
  ...
  return <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full flex flex-row items-center justify-center gap-4">
        ...
        <motion.div className="flex-1 bg-gradient-to-r from-teal-500 to-cyan-600 rounded-lg shadow-lg overflow-hidden border-2 border-teal-400" whileHover={{
        scale: 1.02
      }} whileTap={{
        scale: 0.98
      }} initial={{
        ...
        {/* CONDUCE button */}
        <motion.div className="flex-1 bg-gradient-to-r from-teal-500 to-cyan-600 rounded-lg shadow-lg overflow-hidden border-2 border-teal-400" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        scale: 0.98
          ...
          <Link to="/motorcycle-drive-game" onClick={handleNavigation} className="block w-full py-2 px-3 text-white relative">
      ...
      <AnimatePresence>
        ...
        duration: 0.3
          ...
          return <motion.div key={motorcycle.id} className={`relative flex flex-col items-center p-2 rounded-lg cursor-pointer transition-all duration-200
                    ${isSelected ? 'bg-teal-100 ring-2 ring-teal-500' : 'hover:bg-teal-50'}
                  `} onClick={() => handleCarSelect(motorcycle)} whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
          }}>
                  <div className="relative">
                    {/* Motorcycle image */}
                    <img src={`/lovable-uploads/${motorcycle.image}`} alt={motorcycle.name} className="w-16 h-16 object-contain" />
                    ...
                    {/* Selected indicator */}
                    {isSelected && <motion.div className="absolute -right-2 -top-2 w-6 h-6 bg-teal-500 rounded-full flex items-center justify-center" initial={{
                scale: 0
              }} animate={{
                scale: 1
              }} transition={{
