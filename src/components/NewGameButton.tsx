...
const NewGameButton: React.FC = () => {
  ...
  return <motion.div className="text-center space-y-2 bg-white p-3 rounded-lg shadow-lg w-full max-w-xs mb-2" whileHover={{
    scale: buttonDisabled ? 1 : 1.03
  }} transition={{
    type: "spring",
    stiffness: 300
  }}>
      <Button onClick={handleClick} disabled={buttonDisabled} className={`px-4 py-4 ${buttonDisabled ? 'bg-gray-400 cursor-not-allowed' : `bg-gradient-to-r ${buttonGradient}`} w-full ${isAnimating ? "animate-bounce" : ""}`}>
        <div className="relative w-full flex items-center justify-center">
          {/* Slot machine reel effect */}
          <div className="flex gap-1 mr-2">
            {isAnimating ? <>
                <motion.span className="inline-block text-lg font-bold" animate={{
              y: [-20, 20],
              opacity: [0, 1, 0]
            }} transition={{
              duration: 0.3,
          ...
          <span className="font-medium text-2xl">
            {isAnimating ? <div className="flex items-center">
