...
import { Star, Globe, Medal, Trophy, Map, Route } from "lucide-react";
    ...
    <div className="w-full grid grid-cols-2 gap-4 mb-4">
      ...
      >
        <div className="flex items-center justify-center gap-2">
          ...
          >
            {/* Changed to Route icon for kilometers */}
            <div className="h-14 w-14 flex items-center justify-center relative">
              <Route className="h-12 w-12 text-green-500 absolute" strokeWidth={1.5} />
              <motion.div 
                className="absolute inset-0" 
                animate={{ scale: [1, 1.2, 1], opacity: [0.7, 1, 0.7] }}
                transition={{ repeat: Infinity, duration: 2 }}
              >
                <Map className="h-12 w-12 text-green-400" strokeWidth={1} />
              </motion.div>
              <Star className="h-5 w-5 text-amber-400" fill="gold" stroke="orange" />
            </div>
          </motion.div>
          <h3 className={`text-xl ${textColor} kids-text font-normal`}>{t('kilometers_total')}</h3>
        </div>
        
        <motion.p 
          className={`text-3xl ${textColorDark} kids-text mt-1 font-normal`}
        ...
        <div className="mt-2">
          ...
          <p className={`${textColorLight} mt-1 text-lg`}>
            {500 - totalPoints % 500} {t('kilometers_for_next_level')}
          </p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center mt-1">
          ...
          <p className={`${textColorLight} kids-text text-base font-normal text-center`}>
            {t('you_are_in')} <br/>{originInfo.city}, {originInfo.country}
          </p>
        </div>
      </motion.div>
    </div>
